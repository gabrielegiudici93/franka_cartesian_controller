...
def check_generic(conf, *k, **kw):
    ...
    # path
    include_dirs = ['/usr/include/', '/usr/local/include/', os.environ['HOME'] + '/include', os.path.dirname(os.path.realpath(__file__)) + '/third_party/'+ lib + '/include']
    lib_dirs = ['/usr/lib/', '/usr/local/lib/', os.environ['HOME'] + '/lib', '/usr/lib/x86_64-linux-gnu/', os.path.dirname(os.path.realpath(__file__)) + '/third_party/'+ lib +'/lib']
